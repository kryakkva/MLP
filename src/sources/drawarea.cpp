...
/*
    ...
    void rescaled(QImage *img){
        ...
        for (; i < x; i++)
            for (int k = 0; k < 28; k++)
                tmp.setPixelColor(k, i, img->pixelColor(0, 0));
        for (int j = 0; j < img->height(); i++, j++)
            for (int k = 0; k < 28; k++)
                tmp.setPixelColor(k, i, img->pixelColor(k, j));
        for (; i < 28; i++)
            for (int k = 0; k < 28; k++)
                tmp.setPixelColor(k, i, img->pixelColor(0, 0));
        if (rottate)
...
//		double ret[784];
//		int num = 0;
        QImage visibleImage = image.copy(findCropArea()).scaled(28, 28, Qt::KeepAspectRatio, Qt::SmoothTransformation). \
        ...
        if (visibleImage.width() > visibleImage.height() * 3)
            ...
            visibleImage = visibleImage.scaled(28, 28, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
...
//		int num = 0;
        ...
        if (loadedImage.width() > loadedImage.height() * 3)
            ...
            loadedImage = loadedImage.scaled(28, 28, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
...
//					dbg << color.hue() << color.saturation() << color.lightness() << " | ";
                    if (color.lightness() < 20)
                        color = color.lighter(1000);
//						color.setHsl(color.hue(), color.saturation(), 255);
//					loadedImage.setPixelColor(x, y, QColor::fromHsl(color.hue(), 0, color.lightness()).lighter(500));
                    loadedImage.setPixelColor(x, y, color);
                }
//				dbg << color.lightness();
//				ret[num++] = color.lightness() / 255.;
...
//		for (int y = 0; y < loadedImage.height(); ++y) {
//				QDebug dbg(QtDebugMsg);
//			QRgb *line = reinterpret_cast<QRgb*>(loadedImage.scanLine(y));
//			for (int x = 0; x < loadedImage.width(); ++x) {
//				QRgb &rgb = line[x];
//				if (rgb == bg)
//					rgb = qRgb(0, 0, 0);
